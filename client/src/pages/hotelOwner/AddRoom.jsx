...
const AddRoom = () => {
  ...
  return (
    <form>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key)=>(
          <label htmlFor={`roomImage${key}`} key={key}>
            <img src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="image" className='max-h-13 cursor-pointer opacity-80' />
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select value={inputs.roomType} onChange={e=>setInputs({...inputs, roomType: e.target.value})} className='border opacity-70 border-gray-300 mt-1 rounded p-2 w-full'>
            ...
            <option value="Luxuxry Bed">Luxuxry Bed</option>
      ...
      <button className='bg-primary text-white px-8 py-2 rounded mt-2 cursor-pointer'>
        Add Rooms
      </button>
    </form>
  )
