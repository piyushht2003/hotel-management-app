  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pb-30 pt-20'>
      <div className='flex flex-col md:flex-row items-center justify-between w-full'>
        ...
        <button className='group flex items-center gap-2 max-md:mt-12 font-medium transition-all cursor-pointer'>
            View All Offers
            <img 
      ...
      <div className='mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full'>
        {exclusiveOffers.map((item)=>(
            <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
                <p className='px-3 py-1 absolute top-4 left-4 text-xs bg-white text-black text-gray800 font-medium rounded-full'>{item.priceOff}% OFF</p>
                <div>
